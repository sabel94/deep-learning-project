...
from network.model.accuracy import accuracy_ab, accuracy_probs_wrapper
...
def train(params, nn_learner):
    ...
    acc_probs_threshold = c_training['accuracy_threshold']
    apw = accuracy_probs_wrapper(acc_probs_threshold)
    co = {'weighted_categorical_crossentropy': wccw,
          'accuracy_probs': apw
         }
    if c_training['load_model'] == 1 and os.path.isfile(dir_model):
        print("Load model:")
        model = load_model(dir_model, custom_objects=co)
    else:
        # Get model
        network = c_training['network']
        model = models.get_model(network, c_data)
...
def test(params, nn_learner, pts_in_hull):
    ...
    index = 0
    accuracy_sum = 0
    acc_ab_threshold = c_testing['accuracy_threshold']
    dir_output = c_data['dir']['output']
    for batch in datagen_testing:
        # Extract batches
        batch_input = batch[0]      # Normalized L-channel, range [0, 1]
        t_batch_probs = batch[1]    # Ground truth probabilities for ab bins
        t_batch_color_ab = batch[2] # Ground truth image (ab channel) 
        ...
        p_batch_color_ab = data.z2y(p_batch_probs, 
                                    pts_in_hull, 
                                    [c_data, c_testing]
                                    ) # (batch_size, H, W, 2)
        ...
        images_lab = np.concatenate((batch_input, p_batch_color_ab), axis=3)
    
        # Save images
        helpers.save_images(images_lab, dir_output, index)
        
        # Calculate accuracy and sum it up with the accuracy of previous
        # batches
        accuracy_sum += accuracy_ab(t_batch_color_ab, p_batch_color_ab, 
                                    acc_ab_threshold
                                    )
        ...
        if index == n_batches - 1:
            # Calculate and print out average accuracy
            print("Average accuracy: ", accuracy_sum / n_batches)
            return
        else:
            index = index + 1
